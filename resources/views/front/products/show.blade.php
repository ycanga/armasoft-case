...
@section('content')
    <div class="p-4 sm:ml-64">
                ...
                <div class="container px-5 pt-5 grid grid-cols-2 gap-4 content-center">
                    ...
                    <div class="lg:w-1/2 w-full lg:pl-10 lg:py-6 lg:mt-0 pl-5">
                        <h2 class="text-sm title-font text-gray-500 tracking-widest">
                            {{ optional($product->category->first())->name }}</h2>
                        <h1 class="text-gray-900 text-3xl title-font font-medium mb-1">The Catcher in the Rye</h1>
                        <ul class="leading-relaxed text-sm mt-3">
                            <li><strong>SKU:</strong> {{ $product->sku }}</li>
                            <li><strong>ASIN:</strong> {{ $product->asin ?? '-' }}</li>
                            <li><strong>MARKETPLACES:</strong> {{ $product->marketplaces->first()->name }}</li>
        ...
        @if (count($product->issues))
            <div class="p-5">
                <div role="alert">
                    <div
                        class="@if ($product->issues->first()->status == 'ERROR') bg-red-500 @else bg-orange-500 @endif text-white font-bold rounded-t px-4 py-2">
                        {{ $product->issues->first()->status }}
                    </div>
                    <div
                        class="border border-t-0 @if ($product->issues->first()->status == 'ERROR') border-red-400 bg-red-100 text-red-700 @else text-orange-500  border-orange-500 bg-orange-100 @endif  rounded-b  px-4 py-3 ">
                        <p>{{ $product->issues->first()->details }}</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div id="default-tab-content">
            <div class="hidden p-4 rounded-lg bg-gray-50 dark:bg-gray-800" id="details" role="tabpanel"
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    <strong>Store:</strong> {{ $product->store->first()->name }} <br>
                    ...
                    <strong>Barcode: </strong> {{ $product->listings->first()->barcode ?? "---" }} <br>
                    <strong>F-channel: </strong> {{ $product->listings->first()->f_channel ?? "---" }} <br>
                    <strong>Browse Node: </strong> {{ $product->listings->first()->browse_node ?? "---" }} <br>
